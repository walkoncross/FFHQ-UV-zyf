...
def detect_68p(img, five_points, sess, input_op, output_op):
    ...
    landmark = sess.run([output_op.name], input_feed={input_op.name: input_img})[0]
...
def load_lm_graph(graph_filename):
    if not graph_filename.endswith('.onnx'):
        graph_filename = osp.splitext(graph_filename)[0] + '.onnx'
    ...
    lm_sess = onnxruntime.InferenceSession(graph_filename, providers=providers)

    img_224 = lm_sess.get_inputs()[0]
    output_lm = lm_sess.get_outputs()[0]
